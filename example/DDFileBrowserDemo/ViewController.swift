    ...
    @objc func _writeData() {
        ...
        for i in 0..<300000 {
            ...
            if let path = path, let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                do {
                    try data.write(to: DDUtils.shared.getFileDirectory(type: .caches).appendingPathComponent("3333_\(i).jpg"))
                } catch  {
                    print(error)
                }
            }
...
//        DDFileBrowser.shared.rootDirectoryPath = DDUtils.shared.getFileDirectory(type: .caches)
