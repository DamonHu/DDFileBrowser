...
open class DDFileBrowser: NSObject {
    private static let instance = DDFileBrowser()
    open class var shared: DDFileBrowser {
        return instance
    }
    public var rootDirectoryPath = DDUtils.shared.getFileDirectory(type: .home)
    //MARK: UI
    lazy var mNavigationController: UINavigationController = {
        let rootViewController = DDFileBrowserVC()
...
public extension DDFileBrowser {
    func start() {
    ...
    func getFileType(filePath: URL?) -> DDFileType {
...
private extension DDFileBrowser {
